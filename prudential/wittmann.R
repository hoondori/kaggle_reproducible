...
# seperate categorical, continous and discrete variables
...
# we saw missing data, how much is it?
sum(is.na(train)/(nrow(train)*ncol(train))) # for entire row/col
sum(is.na(test)/(nrow(test)*ncol(test)))

apply(train, 2, function(x) { sum(is.na(x))}) # for each of column
apply(test, 2, function(x) { sum(is.na(x))}) # for each of column


# Can we see any different missing data structure depending on the response?
train.na.per.response <- 
  sapply(
    sort(unique(train$Response)), 
    function(x) {
      apply(
        train[train$Response == x, ], 
        2, 
        function(y) {
          sum(is.na(y))
        }
...
# Plot the missingness structure (only for those features with missing data) 
# where the feature with the most missing data is on top and with the least missing data on bottom
plotMissingStr <- function(data.in, title=NULL) {
  r <- as.data.frame(ifelse(is.na(data.in),0,1))
  r <- r[, order(colMeans(is.na(data.in)))]
  data.tmp <- expand.grid(list(x=1:nrow(r),y=colnames(r)))
  data.tmp$r <- as.vector(t(r))
  ggplot(data.tmp) + 
...
plotMissingStr(data.in=train[,apply(train,2,function(x) { sum(is.na(x)) > 0 })], title="Train data set")
plotMissingStr(data.in=test[,apply(test,2,function(x) { sum(is.na(x)) > 0 })], title="Test data set")
...
ggplot(train) + 
  geom_histogram(aes(factor(Response))) + 
  xlab("Response") + 
  theme_light()
